...
- [JSON-Render / HarnessAgent](../tool/tool-json-render.md) — 同一波「生成式 UI」思路，定位略偏「实验性 sandbox 渲染」；CopilotKit 更偏生产化 React 集成。
- [Archify](../tool/tool-archify.md) — 同样 LLM 输出结构化 JSON → 渲染器消费的范式（架构图方向）。
- [Claude Code](../tool/tool-claude-code.md) — 终端 agent；CopilotKit 让「agent 渲染 UI」这件事在 web 前端落了地。

## 相关概念

- [JSON-Render / 生成式 UI](../tool/tool-json-render.md) — 同范式，sandbox 偏实验；CopilotKit 偏生产 React 集成
- [Archify](../tool/tool-archify.md) — 同 LLM→JSON→渲染 思路，产物是 SVG 架构图
- [Claude Code](../tool/tool-claude-code.md) — CopilotKit 不直接接 Claude Code，但生成式 UI 范式同样适用 agent 输出的渲染
