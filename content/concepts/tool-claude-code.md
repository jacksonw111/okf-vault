...
Claude Code 是本知识库的**默认生产环境**——`PRODUCER.md` 协议就是写给它（或类似 agent）看的：`PRODUCER.md` 把它当作 inbox 消费者，能读文件、能写文件、能改 frontmatter、能归档，已经在 [在 Obsidian 里开始用 OKF](playbook-okf-obsidian-start.md) 的「自动同步」一节里被推荐。
