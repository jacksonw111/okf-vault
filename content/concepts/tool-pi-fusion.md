...
- [pi-task](concepts/tool-pi-task-delegation.md) — 同属 Pi 生态扩展（pi-task 偏子任务委派，pi-fusion 偏多模型汇总）
- [Pi Web Agent](concepts/tool-pi-web-agent.md) — 同样为 Pi 加能力（联网工具）
- [Brigade](concepts/tool-brigade.md) — 同样是「多 AI 协同工作」的方案（Brigade 走多 Agent，pi-fusion 走多模型）